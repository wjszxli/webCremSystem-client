...
export default Form.create()(
  class Customer extends React.Component {
    ...
    getPageData = (pageIndex) => {
      ...
      // 获取分页的数据
      let url = `${config.apiUrl}/getPlan?pageSize=10&isDelete=0&model=0&pageIndex=${pageIndex}${searchItem}`
        ...
        <div style={{ marginBottom: '10px' }}>
          ...
          </Button>
          {
            (this.state.isMaster || this.state.isFinance) && (
              <span>
                <Button type="primary" onClick={() => this.updatePlanBack(this.state.isBack)}>
