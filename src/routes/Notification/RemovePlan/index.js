...
export default Form.create()(
  class Customer extends React.Component {
    ...
    getPageData = (pageIndex, searchData) => {
      ...
      // 获取分页的数据
      let url = `${config.apiUrl}/getPlan?pageSize=10&tag=all&isDelete=1&model=1&pageIndex=${pageIndex}${searchItem}`
      ...
      // 获取数据条数
      url = `${config.apiUrl}/getPlanCount?isDelete=1&model=1&tag=all${searchItem}`
      request(url).then(res => {
        if (res) {
          if (res.code === 0) {
            this.setState({
