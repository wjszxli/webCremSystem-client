...
export default Form.create()(
  class Resource extends React.Component {
    ...
    getPageData = (pageIndex, searchData) => {
      ...
      API.getPublicNumberData(pageIndex, searchItem).then(({ listData, dataCount }) => {
    ...
    getOnePublicNumber = async () => {
      ...
      const editData = await API.getOnePublicNumberData(id)
      this.setState({
        editData,
        isAdd: false,
      })
    ...
    generateDate = ({ header, results }) => {
      ...
      results.forEach(item => {
        ...
        API.savePublicNumber(data).then(({ tip }) => {
          message.success(tip)
        })
    ...
    deletePublicNumber = async () => {
      ...
      const { id } = listData[selectedRowKeys[0]]
      const { tip } = await API.deletePublicNumber(id)
      if (tip) {
        message.success(tip)
        this.getPageData(1)
        this.setState({
        ...
        <div style={{ marginBottom: '10px' }}>
          {
            isAuthFlag && (
              <span>
                ...
                <Button type="primary" onClick={this.imageUpdate}>粉丝属性</Button>
                ...
                <Button type="primary" onClick={this.deletePublicNumber}>删除</Button>
              </span>
            )}
            ...
            <Col md={8} sm={24}>
              <FormItem label="排序">
                {getFieldDecorator('order', {
                  ...
                  <Select style={{ width: '90%' }} placeholder="选择刷号搜索" onSelect={(val) => this.searchData(val)}>
