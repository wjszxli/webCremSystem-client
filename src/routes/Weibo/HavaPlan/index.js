...
import { Table, Card, Form, Row, Col, Input, Button, Select, message, DatePicker, Pagination, Popover, Modal } from 'antd';
...
const allAuthers = [{ 'master': false }, { 'medium': false }, { 'charge': false }, { 'sell': false }, { 'finance': false }]
...
export default Form.create()(
  class Customer extends React.Component {
    ...
    updatePlan = (type) => {
      const { selectedRowKeys, listData, time, isPay, isBack, current } = this.state
      let title = ''
      // eslint-disable-next-line no-plusplus
      ...
      return (
        <div style={{ marginBottom: '10px' }}>
          ...
          </Button>
          {
            isFinance && (
              <span>
                <Button type="primary" onClick={() => this.updatePlanBack(isBack)}>
                  {isBack === 1 ? '撤销回款' : '勾选回款'}
                </Button>
    ...
    searchArea() {
      ...
      const optionMedium = user.map(d => <Option value={d.name}>{d.name}</Option>);
            ...
            <Col md={8} sm={24}>
              <FormItem label="排期搜索">
                {getFieldDecorator('publicNumber', {
                  initialValue: '',
                })(<Input placeholder="可搜索id、客户、微博" style={{ width: '90%' }} />)}
              </FormItem>
            </Col>
            <Col md={8} sm={24}>
            ...
            <Col md={8} sm={24}>
              <FormItem label="排序">
                {getFieldDecorator('order', {
                  ...
                  <Select style={{ width: '90%' }} onSelect={(val) => this.searchData(val)}>
    ...
    render() {
      const { selectedRowKeys, totalData, timeVisiable, visibleRemark, listData, editData, isAdd, current, dataCount, visible } = this.state;
      ...
      const columns = [
        ...
        },
        {
          ...
          key: 'backTime',
          render: val => {
            if (Number(val)) return <span>{moment(Number(val)).format('YYYY-MM-DD')}</span>
            return <div></div>
          },
        },
        {
      ...
      return (
        <PageHeaderLayout title="微博排期">
          <Modal
            title="选择回款时间"
            visible={timeVisiable}
            onCancel={() => { this.setState({ timeVisiable: false }) }}
            onOk={() => { this.updatePlan('back') }}
          >
            回款时间：<DatePicker allowClear onChange={this.onTimerChange} />
          </Modal>
          <Card bordered={false}>
